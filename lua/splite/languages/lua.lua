...
--[[lt
# 1. 全体概要
このファイルは，luaファイル（拡張子が`.lua`）に対する*Literate Mode*を提供する
設定ファイルである．

基本方針
- 他言語と見た目（背景，テキスト色など）を揃える
- luaのコメントブロックの開始行に`lt`を付与することで，`literate mode`を識別する
- literate modeでは，ltコメントブロック以外のシンタックスハイライトは無視される
...
return {
  ...
  find_literate_range = function(buf, target_line)
    ...
    if debug_file then
      ...
      -- オプション: エラーログ出力
      print("Debug log open failed: " .. (err or "unknown error"))
    end
  ...
  highlight_function = function(buf, ns, first, last)
    ...
    for l, line in ipairs(lines) do
      ...
      if in_literate and not string.find(line, "%-%-%[%[lt") and not string.find(line, "%]%]") then
        ...
        if h4_start then
          vim.api.nvim_buf_add_highlight(buf, ns, "SpliteLuaH4", lnum, h4_start-1, h4_end or -1)
        else
          local h3_start, h3_end = string.find(line, "^%s*###[^#].*$")
          if h3_start then
            vim.api.nvim_buf_add_highlight(buf, ns, "SpliteLuaH3", lnum, h3_start-1, h3_end or -1)
          else
            local h2_start, h2_end = string.find(line, "^%s*##[^#].*$")
            if h2_start then
              vim.api.nvim_buf_add_highlight(buf, ns, "SpliteLuaH2", lnum, h2_start-1, h2_end or -1)
            else
              local h1_start, h1_end = string.find(line, "^%s*#[^#].*$")
              if h1_start then
                vim.api.nvim_buf_add_highlight(buf, ns, "SpliteLuaH1", lnum, h1_start-1, h1_end or -1)
              end
            end
          end
        end
        ...
        while true do
          ...
          vim.api.nvim_buf_add_highlight(buf, ns, "SpliteLuaBold", lnum, start_pos-1, end_pos or -1)
        ...
        while true do
          ...
          if before_char ~= "*" and after_char ~= "*" then
            vim.api.nvim_buf_add_highlight(buf, ns, "SpliteLuaItalic", lnum, start_pos-1, end_pos or -1)
          end
        ...
        while true do
          ...
          if not start_pos then break end
          vim.api.nvim_buf_add_highlight(buf, ns, "SpliteLuaCodeInline", lnum, start_pos-1, end_pos or -1)
